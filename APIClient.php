...
class APIClient {
    ...
    public static function getUser($id) {
        ...
        foreach ($user_array as $u) {
            $users[] = new User(
                    $u->{'ID'}, $u->{'userName'}, $u->{'firstName'}, $u->{'lastName'}, $u->{'admin'}, $u->{'notify'}
            );
        }
        if ($id != null)
            return $users[0];
    ...
    public static function getAnnouncements($num) {
        ...
        foreach ($json_array as $item) {
            $announcements[] = new Announcement(
                    $item->{'userID'}, $item->{'title'}, $item->{'content'}, $item->{'deptID'}, $item->{'tStamp'}
            );
        }
    ...
    public static function getLogs($logID, $user, $course, $startTime, $endTime) {
        ...
        foreach ($json_array as $item) {
            $logs[] = new Log(
                    $item->{'ID'}, $item->{'userID'}, $item->{'courseID'}, $item->{'comments'}, $item->{'tStamp'}
            );
        }
        return $logs;
    }
    ...
    public static function getPunchCards($punchcardID, $userid, $checkedIn, $startTime, $endTime) {
        ...
        foreach ($json_array as $item) {
            $punchcards[] = new PunchCard(
                    $item->{'ID'}, $item->{'userID'}, $item->{'checkedIn'}, $item->{'tStamp'}
            );
        }
        return $punchcards;
    }
    ...
    public static function getCourses($courseID, $deptID) {
        ...
        foreach ($json_array as $item) {
            $courses[] = new Course(
                    $item->{'ID'}, $item->{'courseName'}, $item->{'deptID'}
            );
        }
        return $courses;
    }
    ...
    public static function getLocations($locID, $buildingName, $roomNumber) {
        ...
        foreach ($json_array as $item) {
            $locations[] = new Location(
                    $item->{'ID'}, $item->{'buildingName'}, $item->{'roomNumber'}
            );
        }
        if ($locID != NULL)
            return $locations[0];
    ...
    public static function getTimeSlots($tSlotID, $locID, $deptID, $courseID, $startTime, $endTime) {
        ...
        foreach ($json_array as $item) {
            $tSlots[] = new TimeSlot(
                    $item->{'ID'}, $item->{'locID'}, $item->{'deptID'}, $item->{'courseID'}, $item->{'startTime'}, $item->{'endTime'}
            );
        }
        if ($tSlotID != NULL)
            return $tSlots[0];
    ...
    public static function getSurveys($surveyID, $courseID, $tutorID, $rating, $viewed) {
        ...
        foreach ($json_array as $item) {
            $surveys[] = new Survey(
                    $item->{'ID'}, $item->{'courseID'}, $item->{'tutorID'}, $item->{'rating'}, $item->{'title'}, $item->{'comment'}, $item->{'viewed'}
            );
        }
        if ($surveyID != NULL)
            return $surveys[0];
    ...
    public static function getFiles($courseID, $fileName) {
        ...
        foreach ($json_array as $item) {
            $files[] = new KnowledgeFile(
                    $item->{'ID'}, $item->{'courseID'}, $item->{'userID'}, $item->{'filename'}, $item->{'content'}, $item->{'approved'}
            );
        }
        return $files;
    }
